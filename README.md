...
- a shared [`matplotlibrc`](matplotlibrc) with compact font sizes, thin lines, vector PDF output, and a colorblind-friendly palette
- a small helper module in [`utils/plot_utils.py`](utils/plot_utils.py) for consistent figure sizing
- an example plot script in [`refine_score.py`](refine_score.py)
- a cropping utility in [`fig/autocrop.sh`](fig/autocrop.sh) for trimming generated PDFs
...
This writes the output figure to [`fig/refine_score_plot.pdf`](fig/refine_score_plot.pdf).
...
[`matplotlibrc`](matplotlibrc) is picked up automatically when you run scripts from the repository root. It defines the default appearance for all plots in this project, including:
...
[`utils/plot_utils.py`](utils/plot_utils.py) provides `set_size(...)` so figures match common paper widths without trial and error.
...
[`refine_score.py`](refine_score.py) shows the intended workflow:
...
1. copy [`refine_score.py`](refine_score.py) to a new script
...
- `plt.savefig("fig/name")` produces a PDF because [`matplotlibrc`](matplotlibrc) sets `savefig.format: pdf`.
